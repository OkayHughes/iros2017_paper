...
Nevertheless, we addressed this concern by adding a comparison to the MHD which is summarized in the Figure \ref{MHD} below and included in the resubmitted version of our manuscript. We outperform \cite{Kitani2012} and \cite{Alahi2016} in the intermediate term, but at short times \cite{Alahi2016} puts more weight on the direction of the observed trajectory, and so their narrow distribution is closer to the trajectory. At large scales, \cite{Kitani2012} outperforms us because they know the endpoint of the trajectory. If you look at Figure \ref{bookstore} and \ref{death}, you'll see that while they are geometrically closer to the observed trajectory, they don't include it.
...
\begin{figure}
	...
	\caption{A comparison of the AUC of the various algorithms. Note that the initial dip in the performance of \cite{Kitani2012} is due to their confidence in their initial estimate. We sampled the S-LSTM \cite{Alahi2016} model $100$ times in order to organically extract a less concentrated probability distribution from their algorithm, but their confidence combined with the over-reliance on social forces at moderate-to-large timescales lowered their performance.}
...
The distance used is a custom distance function. Let one trajectory $A$ start at $(x_{A,\mathrm{start}}, y_{A,\mathrm{start}})$ and end at  $(x_{A,\mathrm{end}}, y_{A,\mathrm{end}})$, and another trajectory $B$ start at $(x_{B,\mathrm{start}}, y_{B,\mathrm{start}})$ and end at $(x_{B,\mathrm{end}}, y_{B,\mathrm{end}})$ . 
...
\response{
	We then cluster in $\mathbb{R}^4$ using Affinity Propagation \cite{FreyDueck2007} and a custom distance measure defined on the endpoints of trajectories. 
...
We included the visualizations in Figures \ref{bookstore} and \ref{death} below, also which are also updated in the paper. 
