...
Since the set of plausible human behaviors is vast, this work focuses on anticipating the possible future locations of people within a bounded area. 
...
\begin{figure}
	...
	\begin{subfigure}[b]{.45\linewidth}
		...
		\caption{$t=11.5s$}
	\end{subfigure}
	\caption{The performance of the presented algorithm captures the most probable routes that a pedestrian chooses. In this example which was captured at thirty frames per second, the presented algorithm took 0.0158s per frame in a Python implementation.  In this figure, the dot is the start point of the test trajectory, the diamond is the position at time $t$, and the X is the end of the trajectory.}
	\label{fig:gates-1-2}
\end{figure}

Autonomous systems are increasingly being deployed in and around humans.
 The ability to accurately model and anticipate human behavior is of great value to maximize safety, confidence, and effectiveness in human-centric environments. 
 The stochasticity of humans necessitates a probabilistic approach to capture the likelihood of possible action over a set a of possible behaviors. 
 Since the set of plausible behaviors in arbitrary situations may be vast, this work constrains the problem to anticipating the possible future locations of people within a bounded area. This problem is critical in many application domains such as enabling personal robots to navigate in crowded environments, managing pedestrian flow in smart cities, and synthesizing safe controllers for autonomous vehicles (AV).  
...
Given a regular partition $\{ s_0, s_1, \dots, s_n \}$ of step-size $\Delta s > 0$ on the support of $\Pr(s)$, 
...
where $TV(x_t, \hat{x}_0, \hat{v}_0)$ is the sum, with respect to $k$, of the total variation of $\Pr( x_t, k , s  \mid \hat{x}_0, \hat{v}_0 )$ with respect to $s$ for fixed $x_t, \hat{x}_0, \hat{v}_0$.
Since this error term can be controlled, the problem of solving $\rho_t(x_t)$ is reduced to that of efficiently computing $ \Pr( x_t, k , s_j  \mid \hat{x}_0, \hat{v}_0)$
...
\end{align}
 ...
 we may approximate it as a sum of weighted Dirac-delta distributions supported on a regular grid.
 ...
 Then, for a given resolution $N_x \in \mathbb{N}$ define the regular grid on $S_L(\hat{x}_0)$ as $\Gamma_L( \hat{x}_0 ; N_x ) := \left\{ x_0^{i,j} \mid i,j = -N_x,\dots,N_x \right\}$, where $x_0^{i,j} = \hat{x}_0 + \frac{L}{2N_x}(i,j)$.
 The grid spacing is given by $\Delta x = ( \frac{L}{2N_x}, \frac{L}{2N_x} )$.
Approximate the smooth distribution $\Pr(\hat{x}_0, x_0, k, s) \Psi( \hat{v}_0 ; k, s, x_0 )$ as a weighted sum of Dirac-deltas (in the variable $x_0$) supported on $\Gamma_L( \hat{x}_0;N)$:
\begin{align}
	\begin{split}
...
\end{align}
where $W(k,s,i,j,\hat{x}_0)$ is merely the evaluation of $\Pr(\hat{x}_0, x_0, k, s) \Psi( \hat{v}_0 ; k, s, x_0 )$ at the grid point $x_0 = x_0^{i,j} \in \Gamma( \hat{x}_0; N)$.
%That is to say
%\begin{align}
%	\begin{split}
%	&W(k,s,i,j,\hat{x}_0) =\\
...
The first term is computable by flowing the points of the grid, $\Gamma_L(\hat{x}_0; N_x)$, along the vector field $s X_k$.
...
\begin{thm} \label{thm:error}
	...
	Moreover, $\| \varepsilon_t \|$ is constant in time.
\end{thm}
\begin{proof}
...
\end{proof}
This, allows us to compute $\Phi_{k,s}^t( x_0^{\alpha})$ using computations of $\Phi_{k,1}^t(x_0^{\alpha})$.
...
\begin{thm} \label{thm:main}
	...
	\begin{align}
		\begin{split}
		&\Delta s \sum_{i,j,k,\ell} W(k,s_\ell,i,j,\hat{x}_0) \left. \Pr( x_t \mid \check{x}_t ) \right|_{ \check{x}_t = \Phi_{k,1}^{s_\ell t}( x_0^{i,j}) } \\
...
This is a powerful result, since if $\Pr(s)$ has a support contained within the domain $[-\bar{s}, \bar{s}]$, then to solve for $\Pr( x_t, \hat{x}_0, \hat{v}_0)$  (and thus $\rho_t(x_t)$) at times $0, \Delta t, \dots, N_t \Delta t$, we only need to compute $\Phi_{k,1}^{\ell \Delta t \bar{s}}$ for $\ell = -N_t,\dots,N_t$ once.
This is because, once $\Phi_{k,1}^{\ell \Delta t \bar{s}}$ is computed for $\ell = -N_t, \dots ,N_t$ we can use the partition of $[-\bar{s}, \bar{s}]$ given by $\{ \bar{s} \frac{m}{\ell} \}_{m=-\ell}^{\ell}$ to compute a $\Delta s = \bar{s} / \ell$ accurate approximation for each $\ell$, courtesy of Theorem \ref{thm:main}.
...
\begin{algorithm}[t]
		  ...
		  \label{alg:1}
		  \begin{algorithmic}[1]
		    \Require $N_t \in \mathbb{N},  \{ X_k \}_{k=1}^{n}, \hat{x}_0, \hat{v}_0, \Gamma_L( \hat{x}_0, N_x)$, and the posteriors of \eqref{eq:pgm}.
		    \ForEach {$m \in \{ -\ell,\dots,\ell \}$}
		    \State Compute $W(k,s,i,j,\hat{x}_0)$ for $s = \bar{s} m / \ell$.
		    \EndFor  
		    \State Construct a $\mathcal{O}( \Delta x + \bar{s}/\ell + \varepsilon_{\rm tol})$ approximation of $\Pr(\check{x}_t, \hat{x}_0, \hat{v}_0)$ at $t=\ell \Delta t$ via Theorem \ref{thm:main} \label{alg1:step2}.
		    \State Normalize with respect to $\check{x}_t$ to obtain an approximation of $\Pr(\check{x}_t \mid \hat{x}_0, \hat{v}_0)$.
		    \State Convolve with respect to $\Pr( x_t \mid \check{x}_t)$ to obtain $\rho_t(x_t)$ at $t= \ell \cdot \Delta t$
		  \end{algorithmic}
\end{algorithm}
...
For fixed $k$ $i$ and $j$, the computation of $\Phi_{k,1}^t(x_0^{i,j})$ at $t = -N_t\bar{s}, (1-N_t)\bar{s} ,\dots, N_t \bar{s}$ takes $\mathcal{O}(N_t)$ time using an explicit finite difference scheme and can be done in parallel for each $k \in \{1,\dots, n\}$ and $x_0^{i,j} \in \Gamma_L( \hat{x}_0; N_x)$.
Similarly, computing $W(k,s,i,j,\hat{x}_0)$ constitutes a series of parallel function evaluations over tuples $(k,s,i,j)$, where the continuous variable $s$ is only required at finitely many places in Algorithm \ref{alg:1}.
...
\begin{figure}
	...
	\caption{We behave qualitatively and quantitatively better than kitani in this example. Our algorithm starts out with a dense distribution, which spreads with time, whereas Kitani et al.'s algorithm is very confident in its mistake.}
...
\begin{figure}
	...
	\end{minipage}
	\begin{minipage}[c]{0.3\linewidth}
		\includegraphics[width=\linewidth]{./figures/bookstore/ours_1_2_t=50.png}
	\end{minipage}
	\begin{minipage}[c]{0.3\linewidth}
		\includegraphics[width=\linewidth]{./figures/bookstore/ours_1_2_t=130.png}
	\end{minipage}
	\begin{minipage}[c]{0.3\linewidth}
		\includegraphics[width=\linewidth]{./figures/bookstore/ours_1_2_t=370.png}
	\end{minipage}
	...
	\end{minipage}
	\begin{minipage}[c]{0.3\linewidth}
		\includegraphics[width=\linewidth]{./figures/bookstore/kit_1_2_t=50.png}
	\end{minipage}
	\begin{minipage}[c]{0.3\linewidth}
		\includegraphics[width=\linewidth]{./figures/bookstore/kit_1_2_t=130.png}
	\end{minipage}
	\begin{minipage}[c]{0.3\linewidth}
		\includegraphics[width=\linewidth]{./figures/bookstore/kit_1_2_t=370.png}
	\end{minipage}
	...
	\end{minipage}
	\begin{minipage}[c]{0.3\linewidth}
		\includegraphics[width=\linewidth]{./figures/bookstore/rand_1_2_t=50.png}
	\end{minipage}
	\begin{minipage}[c]{0.3\linewidth}
		\includegraphics[width=\linewidth]{./figures/bookstore/rand_1_2_t=130.png}
	\end{minipage}
	\begin{minipage}[c]{0.3\linewidth}
		\includegraphics[width=\linewidth]{./figures/bookstore/rand_1_2_t=370.png}
	\end{minipage}
	...
	\caption{We predict this trajectory very well, and while Kitani et al. captures the same behaviour, however their motion model doesn't adequately keep up with the ground truth of the pedestrian.}
...
Our test data used the bicycle annotations. 
We did a 2-fold cross validation on 4 different scenes of varying complexity. 
Our analysis used the same partitions of the trajectories for all three predictors.
...
The output distributions of the three algorithms were compared using their integrals over the cells of a suitably fine regular grid over our domain. These integrals are used to visualize the distributions in Figures \ref{fig:death-1-2} and \ref{fig:bookstore-1-2}.
In keeping with our measurement model, the ground truth compared against was a grid that was identically zero except on a bounding box around the measurement at time $t$. The dimensions of the bounding box were the average bounding box dimensions in the training data set. Because our predictions all share the same scale, we were able to amalgamate all of the prediction and truth values for all of the simulated agents at a given time step and generate ROC curves. Figure \ref{fig:auc_vs_time} shows the Area Under the Curve of these ROC curves versus time.
\begin{figure}
	\centering
	\includegraphics[width=\linewidth]{./figures/the_results.eps}
	\caption{We see that at all times we outperform Random Walk and Kitani et al.'s algorithm. The initial dip in Kitani et al.'s algorithm is due to their initial distribution being very sharp.}
...
The run time per frame for each algorithm was generated using total run time for 400 frames, averaged across several agents and scenes, divided by the number of frames. This is shown in Table~\ref{tab:time}. Our algorithm implementation leveraged its embarassing parallelism using a relatively naive parallelization scheme, which split the computation of frames among 18 cores. It's highly probable that our run times would significantly decrease using an implementation of our algorithm in C that uses a more clever scheme. Kitani et al. was timed with minimal modification to the source code provided by the authors. 

Figure \ref{fig:auc_vs_time} shows that at all time scales our algorithm has better predictive capability than Kitani et. al's. We also run over 3 times faster than their algorithm, at over 60FPS.  Figures \ref{fig:death-1-2} and \ref{fig:bookstore-1-2} show the qualitative superiority of our predictions on several scenes.
