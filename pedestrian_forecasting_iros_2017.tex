...
\begin{algorithm}[t]
		  \caption{Algorithm to Compute  $\rho_t$ at times $t=0,\Delta t, 2 \Delta t, \dots, N_t \Delta t$.}
		  \label{alg:1}
		  \begin{algorithmic}[1]
		    \Require $N_t \in \mathbb{N}, \Delta t > 0,   \{ X_k \}_{k=1}^{n}, \hat{x}_0, \hat{v}_0, \Gamma_L( \hat{x}_0, N_x)$, and the posteriors of \eqref{eq:pgm}.
		    \For {$\ell = 0,\dots,N_t$}
		    \For {$m = -\ell,\dots,\ell$}
