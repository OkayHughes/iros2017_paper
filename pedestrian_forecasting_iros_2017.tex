...
This is a powerful result, because it allows us to solve for $\Pr( x_t, \hat{x}_0, \hat{v}_0)$  (and thus $\rho_t(x_t)$) at times $t=\Delta t, \dots, N_t \Delta t$ by a single computation of $\Phi_{k,1}^{\ell \Delta t \bar{s}}( x_0^{i,j})$ for each $\ell \in  \{-N_t,\dots,N_t\}$, $k \in \{1,\dots,n\}$, and $x_0^{i,j} \in \Gamma_L( \hat{x}_0 , N_x)$.
To explain this, assume we are given $\Phi_{k,1}^{m \Delta t \bar{s}}(x_0^{i,j})$ for all $|m| \leq \ell$.
...
The partition $\mathcal{P}_\ell$ has a width of size, $\Delta s = \bar{s} / \ell$, and substitution in \eqref{eq:approximation 3} yields an approximation of $\Pr( x_t, \hat{x}_0, \hat{v}_0)$ given by
\begin{align*}
	\begin{split}
	...
	\end{split}
\end{align*}
As we've already computed $\Phi_{k,1}^{m \bar{s} \Delta t}$ for $| m | \leq \ell$ by assumption, the only obstacle to computing this sum is the computation of the weights $W(k,m \bar{s} / \ell ,i,j,\hat{x}_0)$.
...
\begin{algorithm}[t]
		  \caption{Algorithm to Compute  $\rho_t$ at times $t= \Delta t, 2 \Delta t, \dots, N_t \Delta t$.}
		  \label{alg:1}
		  \begin{algorithmic}[1]
		    \Require $\bar{s} > 0, N_t \in \mathbb{N}, \Delta t > 0,   \{ X_k \}_{k=1}^{n}, \hat{x}_0, \hat{v}_0, \Gamma_L( \hat{x}_0, N_x)$,
		    routines for $\Pr(x_0 \mid M), \Pr(x_0 \mid \hat{x}_0), \Pr(v_0 \mid \hat{v}_0 ), \Pr(M \in \mathcal{M}), \Pr(s \mid k)$ .
		    \For {$\ell = 1,\dots,N_t$}
		    \State Compute and store $\Phi_{k,1}^{ \pm \bar{s} \ell \Delta t}( x_0^{i,j} )$ for all $x_0^{i,j} \in \Gamma_L( \hat{x}_0 ; N_x)$.
		    \For {$m = -\ell,\dots,\ell$}
		    \State Compute $W(k,s,i,j,\hat{x}_0)$ for $s = \bar{s} m / \ell$ using \eqref{eq:pgm}.
		    \EndFor  
		    ...
		    with partition $\mathcal{P}_\ell$ of \eqref{eq:partition}.
		   \label{alg1:step2}.
